...
def poly(x):
    ...
    c = np.concatenate([np.ones((1, 1), dtype=complex),
                       np.zeros((1, n), dtype=complex)], axis=1)
...
def activlev(signal, fs, mode='n'):
    ...
    if ns:
        if md != '0':
            sq, configs.zl[0] = scipy.signal.lfilter(
                configs.abl[0, :2], configs.abl[1, :2], signal, zi=[configs.zl[0]])
            sq, configs.zl[1:3] = scipy.signal.lfilter(
                configs.abl[0, 2:5], configs.abl[1, 2:5], sq, zi=np.squeeze(configs.zl[1:3]))
            sq, configs.zl[3:5] = scipy.signal.lfilter(
                configs.abl[0, 5:8], configs.abl[1, 5:8], sq, zi=np.squeeze(configs.zl[3:5]))
        if md != 'h':
            sq, configs.zh = scipy.signal.lfilter(np.squeeze(
                configs.bh), np.squeeze(configs.ah), sq, zi=np.squeeze(configs.zh))

        configs.ns = configs.ns + ns
        configs.ss = configs.ss + sum(sq)
        configs.ssq = configs.ssq + sum(sq * sq)

        s, configs.ze = scipy.signal.lfilter(
            [1], configs.ae, np.abs(sq), zi=configs.ze)
        ...
        if configs.emax == -np.inf:
            ...
            _kc = scipy.sparse.csr_matrix((np.squeeze(wqe), (qe, np.squeeze(wqe))), shape=[
                                          nbin+1, nbin+1]).toarray()
    ...
    if configs.ns:
        if configs.ssq > 0:
            ...
            cj = 10 * np.log10(2) * (configs.emax -
                                     ([i for i in range(1, nbin+1)]) - 1)
            mj = aj.T - cj - thresh
